...
export default async function PaysPage({
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      <h1 className="text-lg font-semibold">Pays</h1>
      ...
      <form method="GET" className="flex flex-wrap gap-2 items-end text-sm">
        ...
        <a
          href={`/api/pays/export?start=${start}&end=${end}&groupBy=${groupBy}`}
      ...
      <table className="text-sm w-full">
        <thead>
          <tr className="text-left text-text-muted">
            ...
            <th className="font-normal pr-4">Total pay</th>
          </tr>
        </thead>
        <tbody>
          {grouped.map((g) => (
          ...
          {grouped.length === 0 && (
            <tr>
              <td colSpan={3} className="py-2 text-text-secondary">
                No pay recorded for this range.
              </td>
            </tr>
          )}
        </tbody>
